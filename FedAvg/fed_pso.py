...
EPOCHS = 5  # number of total iteration
...
lr = 0.0025
OPTIMIZER = tf.keras.optimizers.legacy.SGD(lr=lr, momentum=0.9, decay=lr / (EPOCHS * CLIENT_EPOCHS),
                                           nesterov=False)  # lr = 0.015, 67 ~ 69%
...
def write_csv(algorithm_name, list):
    ...
    file_name = file_name.format(drop=DROP_RATE, name=algorithm_name, lr=lr, cli=NUMOFCLIENTS, cli_epoch=CLIENT_EPOCHS,
...
if __name__ == "__main__":
    ...
    for i in range(NUMOFCLIENTS):
        pso_model.append(
            particle(particle_num=i, client=init_model(train_data_shape=x_train.shape[1:]), x_train=client_data[i][0],
